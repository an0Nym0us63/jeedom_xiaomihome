...
@cli.command()
@click.argument("time", type=click.IntRange(0, 1440, clamp=True))
def cron(time):
    """Add cron to turn the bulb off."""
    click.echo("Adding off cron {}...".format(time))
    for bulb in BULBS:
        bulb.cron_add(CronType.off, time)
